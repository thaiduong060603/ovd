"""
run_pipeline.py  —  Day 3 update
Main pipeline: OVD detection → tracking → rule evaluation → event output → incident management.

Changes vs original:
  - Uses RuleEngineV1 (5 operators, Event output) instead of RuleEngine
  - Accepts --camera-id argument
  - Prints Event details when emitted
  - Event log saved to data/events/events.jsonl at end of session
"""
import argparse
import time
import cv2
...
from src.core.rules.rule_engine_core_v1 import RuleEngineV1          # ← Day 3
...
def main():
    parser = argparse.ArgumentParser(description="OVD Watchdog — Full Pipeline (Day 3)")
    parser.add_argument("--input",              required=True,        help="Video file / RTSP URL")
    parser.add_argument("--rule",               required=True,        help="Rule file (.json or .yaml)")
    ...
    print(f"    entered_polygon: {rule.conditions.use_entered_polygon}")
    print(f"    near_class     : {rule.conditions.near_class}")
    print(f"    near_dist_px   : {rule.conditions.near_distance_px}")
    ...
    # ── 5. Rule engine (V1) ───────────────────────────────────────────────────
    print("\n[5/8] Initializing RuleEngineV1...")
    rule_engine = RuleEngineV1(rules=[rule], camera_id=args.camera_id)
    ...
    if args.display:
        if _check_gui_available():
            ...
            print("  ⚠  --display requested but GUI not available")
    ...
    if args.output:
        ...
        if w > 0 and h > 0:
            writer = cv2.VideoWriter(
                args.output, cv2.VideoWriter_fourcc(*"mp4v"), video_source.fps, (w, h)
            )
            print(f"  ✓ Recording output → {args.output}  ({w}x{h})")

    # ── Event log file ────────────────────────────────────────────────────────
    events_log_path = Path("data/events/events.jsonl")
    events_log_path.parent.mkdir(parents=True, exist_ok=True)
    events_log_file = open(events_log_path, "a", encoding="utf-8")
    print(f"  ✓ Event log → {events_log_path}")
    ...
    try:
        while True:
            ...
            if frame_id % args.detection_interval == 0:
                ...
                if n > 0:
                    print(f"\n[DETECT] frame={frame_id}  t={timestamp:.2f}s  → {n} object(s)")

            # ── TRACKING ──────────────────────────────────────────────────────
            stale     = frame_id - last_det_frame
            det_input = detection_cache if stale < args.detection_interval else []
            tracks    = tracker.update(det_input, frame_id, timestamp)

            if frame_id % 30 == 0:
                ids = [t.track_id for t in tracks[:5]]
                print(f"[TRACK]  frame={frame_id}: {len(tracks)} tracks  IDs={ids}")
            ...
            new_events = [
                ...
                if e.event_id not in emitted_event_ids
            ]
            for event in new_events:
                emitted_event_ids.add(event.event_id)
                _print_separator(f"EVENT: {event.event_id}")
                print(event.to_json())
                # Append to JSONL log
                events_log_file.write(event.to_json(indent=None) + "\n")
            ...
            for incident in incidents:
                if incident.state == "confirmed" and incident.incident_id not in recorded_ids:
                    if rule_engine.should_notify(incident, timestamp, rule):
                        _print_separator(f"INCIDENT CONFIRMED: {incident.incident_id}")
            ...
            if frame_id % 100 == 0:
                print(
                    ...
                    f"  tracks={len(tracks)}"
                    f"  incidents={len(rule_engine.get_active_incidents())}"
                    f"  confirmed={len(rule_engine.get_confirmed_incidents())}"
                    f"  events={len(rule_engine.events)}"
                )
...
def _resolve_device(device_arg: str) -> str:
    ...
    if device_arg in ("cuda", "cuda:0", "cuda:1"):
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(f"Device '{device_arg}' requested but CUDA not available.")
        idx = int(device_arg.split(":")[1]) if ":" in device_arg else 0
        print(f"  ✓ Using {device_arg}: {torch.cuda.get_device_name(idx)}")
        return device_arg
    if device_arg == "mps":
        import torch
        if not (hasattr(torch.backends, "mps") and torch.backends.mps.is_available()):
            raise RuntimeError("MPS not available (requires Apple Silicon + macOS 12+)")
        return "mps"
...
def _print_header(args):
    print("=" * 70)
    print("  OVD WATCHDOG  —  Day 3")
    print("=" * 70)
    print(f"  Input    : {args.input}")
    print(f"  Rule     : {args.rule}")
    print(f"  Camera   : {args.camera_id}")
    print(f"  Det. int : every {args.detection_interval} frames")
...
def _print_summary(frame_id: int, tracker, rule_engine, recorded_ids: set):
    ...
    print(f"  Events emitted   : {len(rule_engine.events)}")
    ...
    if confirmed:
        ...
        for inc in confirmed:
            print(f"    [{inc.incident_id}]  track={inc.track_id}"
                  f"  t={inc.confirmed_time:.2f}s  notified={inc.notification_count}x")

    print(f"\n  Recordings → data/incidents/")
    print(f"  Events log → data/events/events.jsonl")
    print("=" * 70)
