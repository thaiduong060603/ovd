...
"""
Rule and Incident data models
Updated Day 3: RuleConditions extended to support all 5 operators.
"""
from dataclasses import dataclass, field
from typing import List, Optional, Dict, Any
import numpy as np
...
class ROI:
    """Region of Interest"""
    enabled: bool
    roi_type: str  # "polygon", "rectangle"
...
class RuleConditions:
    """
    Conditions for triggering an incident.
    Supports all 5 operators defined in rule_engine_core_v1.py:
      - inside_polygon / entered_polygon  (via use_entered_polygon flag)
      - dwell_time_over                   (via dwell_seconds)
      - near_object                       (via near_class + near_distance_px)
      - attribute_missing                 (via require_helmetless)
    """
    dwell_seconds: float
    min_confidence: float
    min_frames: int = 3

    # ── attribute_missing operator ────────────────────────────────────────────
    require_helmetless: bool = False

    # ── entered_polygon vs inside_polygon ─────────────────────────────────────
    # False (default) = inside_polygon: alert while inside zone continuously
    # True            = entered_polygon: alert once on boundary crossing
    use_entered_polygon: bool = False

    # ── near_object operator ──────────────────────────────────────────────────
    near_class: Optional[str] = None          # class name to check proximity against
    near_distance_px: Optional[float] = None  # max pixel distance to trigger
...
class Rule:
    ...
    def from_yaml(cls, yaml_path: str) -> 'Rule':
        ...
        if roi_data and roi_data.get('enabled', False) and roi_data.get('points'):
            roi = ROI(
                enabled=True,
        ...
        # ── Parse conditions (all operator fields) ────────────────────────────
        cond = data.get('conditions', {})
        conditions = RuleConditions(
            dwell_seconds=cond['dwell_seconds'],
            min_confidence=cond['min_confidence'],
            min_frames=cond.get('min_frames', 3),
            require_helmetless=cond.get('require_helmetless', False),
            use_entered_polygon=cond.get('use_entered_polygon', False),
            near_class=cond.get('near_class', None),
            near_distance_px=cond.get('near_distance_px', None),
        )
